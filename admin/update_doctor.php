...
include 'header.php';
$id=$_REQUEST['id'];
$sql="select doctor.*, department.dep_name, users.name, user_id, username, password from doctor INNER JOIN users on users.id=doctor.user_id INNER JOIN department on department.id=doctor.dep_id where doctor.id='$id'"; 
		...
		<section id="Referal-section">
            <div class="container">
               ...
               <form method="post" enctype="multipart/form-data">
                  ...
                  <button type="submit" name="submit" class="add-btn">Update Doctor</button>
               </form>
            </div>
         </section>
      </main>
...
if(isset($_POST['submit']))
{
	...
	$result=mysqli_query($con,$sql);
	$sql="update doctor set dep_id='$department', specialization='$specialization', qualification='$qualification', experience='$experience', designation='$designation', fee='$fee', gender='$gender', contact='$contact', address='$address', image='$img', about='$about' where id='$id'";
	$result=mysqli_query($con,$sql);
	if($result){
		echo '<script>window.location.href="doctor.php"
				alert("Doctor updated successfully")</script>';
	}
	else{
